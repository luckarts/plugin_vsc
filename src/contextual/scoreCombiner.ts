 ...
 */
export class ScoreCombiner implements IScoreCombiner {
  ...
  combineScores(scores: IRelevanceScores, config: IContextualSearchConfig): number {
    ...
    if (Math.abs(totalWeight - 1.0) > 0.001) {
      throw new ContextualRetrievalException(
        'combineScores', 
    ...
    const combinedScore = 
      (scores.semantic * config.semanticWeight) +
  ...
  normalizeScores(results: IContextualSearchResult[]): IContextualSearchResult[] {
    ...
    for (const result of results) {
      result.scores.semantic = this.normalizeScore(
        result.scores.semantic, 
        scoreStats.semantic.min, 
        scoreStats.semantic.max
      );
      
      result.scores.temporal = this.normalizeScore(
        result.scores.temporal, 
        scoreStats.temporal.min, 
        scoreStats.temporal.max
      );
      
      result.scores.spatial = this.normalizeScore(
        result.scores.spatial, 
        scoreStats.spatial.min, 
        scoreStats.spatial.max
      );
      
      result.scores.structural = this.normalizeScore(
        result.scores.structural, 
        scoreStats.structural.min, 
        scoreStats.structural.max
      );
  ...
  applyAdvancedScoring(
    results: IContextualSearchResult[], 
   ...
   */
  private calculateScoreStatistics(results: IContextualSearchResult[]): {
    semantic: { min: number; max: number; avg: number };
    temporal: { min: number; max: number; avg: number };
    spatial: { min: number; max: number; avg: number };
    structural: { min: number; max: number; avg: number };
  } {
   ...
   */
  private getScoreStats(scores: number[]): { min: number; max: number; avg: number } {
   ...
   */
  private enhanceScoreSeparation(score: number): number {
   ...
   */
  private applyDiversityBonus(results: IContextualSearchResult[]): IContextualSearchResult[] {
   ...
   */
  private applyQualityBoost(results: IContextualSearchResult[]): IContextualSearchResult[] {
    for (const result of results) {
