...
export class FileCreator implements IFileCreator {
   ...
   */
  async createFile(action: ICodeAction): Promise<IActionResult> {
    try {
  ...
  async ensureDirectoryExists(filePath: string): Promise<void> {
    try {
      ...
      throw new ActionException('ensureDirectoryExists', `Failed to create directory: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
    }
  }
   ...
   */
  private extractNameFromContent(content: string, language: string): string {
    try {
      ...
      if (content.includes('class ')) return 'NewClass';
      if (content.includes('function ') || content.includes('=>')) return 'newFunction';
      if (content.includes('interface ')) return 'NewInterface';
      if (content.includes('type ')) return 'NewType';
      if (content.includes('React') || content.includes('JSX')) return 'NewComponent';
   ...
   */
  private suggestDirectory(content: string, language: string): string {
   ...
   */
  private prepareFileContent(action: ICodeAction): string {
   ...
   */
  private shouldAddFileHeader(action: ICodeAction): boolean {
   ...
   */
  private generateFileHeader(action: ICodeAction): string {
