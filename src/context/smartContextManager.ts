 ...
 */
export class SmartContextManager implements ISmartContextManager {
  ...
  private readonly contextPreview: ContextPreview;
  ...
  constructor(contextualRetriever: ContextualRetriever) {
    ...
    this.contextPreview = new ContextPreview();
   ...
   */
  async buildContext(query: string, options: IContextOptions = {}): Promise<ISmartContext> {
    try {
      ...
      if (opts.includeDependencies && workspaceContext.activeFile) {
        const dependencies = await this.contextDetector.analyzeCodeDependencies(
          workspaceContext.activeFile.content,
          workspaceContext.activeFile.language
        );
      ...
      const dependencies = workspaceContext.activeFile 
        ? await this.contextDetector.analyzeCodeDependencies(
            workspaceContext.activeFile.content,
            workspaceContext.activeFile.language
          )
        : {
      ...
      throw new ContextException('buildContext', 'Failed to build smart context', error);
    }
  }
   ...
   */
  async previewContext(query: string, options: IContextOptions = {}): Promise<IPreviewData> {
    try {
      // Build context first
      ...
      throw new ContextException('previewContext', 'Failed to preview context', error);
    }
  }
   ...
   */
  async optimizeContext(context: ISmartContext, maxTokens: number): Promise<ISmartContext> {
    try {
      ...
      if (context.metadata.totalTokens > maxTokens * 1.5) {
        ...
        optimizedCode = this.contextOptimizer.applySmartCompression(
          optimizedCode, 
      ...
      // Update metadata
      const updatedMetadata: IContextMetadata = {
        ...context.metadata,
      ...
      throw new ContextException('optimizeContext', 'Failed to optimize context', error);
    }
  }
   ...
   */
  private mergeWithDefaults(options: IContextOptions): Required<IContextOptions> {
    return {
      ...
      compressionLevel: options.compressionLevel || CompressionLevel.MODERATE
    };
  }
   ...
   */
  private getAppliedFilters(options: IContextOptions): string[] {
   ...
   */
  private buildImportContext(imports: any[]): string | null {
   ...
   */
  private buildDependencyContext(dependencies: any): string | null {
   ...
   */
  private buildRecentFilesContext(workspaceContext: any): string | null {
    if (workspaceContext.recentFiles.length === 0) return null;
   ...
   */
  private createSimpleRelevanceBar(relevance: number): string {
