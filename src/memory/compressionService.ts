...
import { COMPRESSION_CONFIG, MEMORY_CONFIG } from './config';

export class CompressionService implements ICompressionService {
  private compressionCache = new Map<string, ICompressionResult>();
  ...
  async shouldCompress(memories: IMemory[]): Promise<boolean> {
    ...
    if (totalSize > MEMORY_CONFIG.compressionThreshold) {
      return true;
    }
    
    // Check if we have too many memories
    if (memories.length > MEMORY_CONFIG.maxMemoriesPerType) {
      return true;
    }
    
    // Check if uncompressed size is significant
    if (uncompressedSize > COMPRESSION_CONFIG.minSizeForCompression) {
      return true;
    }
   ...
   */
  async compressMemory(memory: IMemory): Promise<ICompressionResult> {
    if (memory.compressed) {
      throw new CompressionError('Memory is already compressed');
    }
    
    if (memory.size < COMPRESSION_CONFIG.minSizeForCompression) {
      throw new CompressionError('Memory too small to compress');
    }
    ...
    try {
      ...
      const compressedContent = await this.performIntelligentCompression(
        memory.content, 
        preservedElements
      );
      ...
      throw new CompressionError(
        `Failed to compress memory ${memory.id}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async compressMemories(memories: IMemory[]): Promise<IMemory[]> {
    const compressedMemories: IMemory[] = [];
    
    for (const memory of memories) {
      try {
        if (!memory.compressed && memory.size >= COMPRESSION_CONFIG.minSizeForCompression) {
          const compressionResult = await this.compressMemory(memory);
          
          const compressedMemory: IMemory = {
            ...memory,
  ...
  private extractPreservedElements(content: string): {
    ...
    for (const keyword of COMPRESSION_CONFIG.preservedKeywords) {
    ...
    for (const pattern of COMPRESSION_CONFIG.preservedPatterns) {
  ...
  private async performIntelligentCompression(
    content: string, 
    preservedElements: { keywords: string[]; patterns: string[]; structures: string[] }
  ): Promise<string> {
   ...
   */
  private isImportantLine(line: string, preservedElements: { keywords: string[]; patterns: string[]; structures: string[] }): boolean {
    // Empty lines
    if (!line) {
      return false;
    }
   ...
   */
  private generateSummary(originalContent: string, compressedContent: string): string {
    ...
    const keywords = COMPRESSION_CONFIG.preservedKeywords.filter(keyword => 
   ...
   */
  private getCacheKey(memory: IMemory): string {
