...
exports.IntelligentMemoryManager = void 0;
const vscode = __importStar(require("vscode"));
const guid_typescript_1 = require("guid-typescript");
const types_1 = require("./types");
const storageService_1 = require("./storageService");
const compressionService_1 = require("./compressionService");
const config_1 = require("./config");
class IntelligentMemoryManager {
    constructor(globalStorageUri) {
        this.eventCallbacks = [];
        this.isInitialized = false;
        this.storageService = new storageService_1.StorageService(globalStorageUri);
        this.compressionService = new compressionService_1.CompressionService();
    }
    /**
    ...
    async initialize() {
        ...
        try {
            ...
            if (config_1.MEMORY_CONFIG.autoCompress) {
            ...
            throw new types_1.MemoryError(`Failed to initialize memory manager: ${error instanceof Error ? error.message : 'Unknown error'}`, 'INITIALIZATION_ERROR');
        }
    }
    /**
     * Create a new memory
     */
    async createMemory(content, type, tags = [], metadata = {}) {
        ...
        try {
            ...
            throw new types_1.MemoryError(`Failed to create memory: ${error instanceof Error ? error.message : 'Unknown error'}`, 'CREATE_ERROR');
        }
    }
    /**
     * Get a memory by ID
    ...
    async updateMemory(id, updates) {
        ...
        if (!existingMemory) {
            throw new types_1.MemoryError(`Memory ${id} not found`, 'NOT_FOUND');
        }
        // Validate updates
        if (updates.content) {
        ...
        try {
            ...
            throw new types_1.MemoryError(`Failed to update memory ${id}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'UPDATE_ERROR');
        }
    }
    /**
     * Delete a memory
    ...
    async deleteMemory(id) {
        ...
        try {
            ...
            throw new types_1.MemoryError(`Failed to delete memory ${id}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'DELETE_ERROR');
        }
    }
    /**
     * Search memories
    ...
    async searchMemories(query, filters) {
        ...
        try {
            ...
            throw new types_1.MemoryError(`Failed to search memories: ${error instanceof Error ? error.message : 'Unknown error'}`, 'SEARCH_ERROR');
        }
    }
    /**
    ...
    async compressMemories() {
        ...
        try {
            ...
            vscode.window.withProgress({
                ...
                vscode.window.showInformationMessage(`✅ Compression complete! Saved ${savedSpace} KB of space.`);
            });
        }
        catch (error) {
            throw new types_1.MemoryError(`Failed to compress memories: ${error instanceof Error ? error.message : 'Unknown error'}`, 'COMPRESSION_ERROR');
        }
    }
    /**
     * Optimize storage
    ...
    async optimizeStorage() {
        ...
        try {
            ...
            throw new types_1.MemoryError(`Failed to optimize storage: ${error instanceof Error ? error.message : 'Unknown error'}`, 'OPTIMIZATION_ERROR');
        }
    }
    /**
     * Export memories
    ...
    async exportMemories(options = {}) {
        ...
        if (options.dateRange) {
            filteredMemories = filteredMemories.filter(memory => memory.timestamp >= options.dateRange.start &&
        ...
        switch (format) {
            ...
            default:
                throw new types_1.MemoryError(`Unsupported export format: ${format}`, 'INVALID_FORMAT');
        }
    }
    /**
     * Import memories
    ...
    async importMemories(data, format = 'json') {
        ...
        try {
            ...
            if (format === 'json') {
                ...
                throw new types_1.MemoryError('Markdown import not yet implemented', 'NOT_IMPLEMENTED');
            }
            let imported = 0;
            let skipped = 0;
            let duplicates = 0;
            ...
            throw new types_1.MemoryError(`Failed to import memories: ${error instanceof Error ? error.message : 'Unknown error'}`, 'IMPORT_ERROR');
        }
    }
    /**
     * Create backup
    ...
    validateMemoryInput(content, type, tags) {
        this.validateContent(content);
        if (!Object.values(types_1.MemoryType).includes(type)) {
            throw new types_1.ValidationError('Invalid memory type');
        }
        if (tags.length > config_1.VALIDATION_CONFIG.memory.maxTagsCount) {
            throw new types_1.ValidationError(`Too many tags (maximum ${config_1.VALIDATION_CONFIG.memory.maxTagsCount})`);
        }
        for (const tag of tags) {
            if (tag.length > config_1.VALIDATION_CONFIG.memory.maxTagLength) {
                throw new types_1.ValidationError(`Tag too long: ${tag}`);
            }
            if (!config_1.VALIDATION_CONFIG.tags.allowedCharacters.test(tag)) {
                throw new types_1.ValidationError(`Invalid tag format: ${tag}`);
            }
            if (config_1.VALIDATION_CONFIG.tags.reservedTags.includes(tag.toLowerCase())) {
                throw new types_1.ValidationError(`Reserved tag: ${tag}`);
            }
        }
    }
    validateContent(content) {
        if (!content || typeof content !== 'string') {
            throw new types_1.ValidationError('Content must be a non-empty string');
        }
        if (content.length < config_1.VALIDATION_CONFIG.memory.minContentLength) {
            throw new types_1.ValidationError(`Content too short (minimum ${config_1.VALIDATION_CONFIG.memory.minContentLength} characters)`);
        }
        if (content.length > config_1.VALIDATION_CONFIG.memory.maxContentLength) {
            throw new types_1.ValidationError(`Content too long (maximum ${config_1.VALIDATION_CONFIG.memory.maxContentLength} characters)`);
        }
    }
    validateMemoryObject(memory) {
        if (!memory || typeof memory !== 'object') {
            throw new types_1.ValidationError('Invalid memory object');
        }
        const required = config_1.VALIDATION_CONFIG.memory.requiredFields;
        for (const field of required) {
            if (!(field in memory)) {
                throw new types_1.ValidationError(`Missing required field: ${field}`);
            }
        }
    ...
    startAutoCompression() {
        ...
        }, config_1.MEMORY_CONFIG.backupInterval);
    }
    ...
    async cleanupOldSessionMemories() {
        const sessionMemories = await this.getMemoriesByType(types_1.MemoryType.SESSION);
