 ...
 */
class ChatWebview {
    constructor(context, claudeClient, vectorDb) {
        this.context = context;
        this.claudeClient = claudeClient;
        this.vectorDb = vectorDb;
        // Initialize action managers
        this.actionExecutor = new actions_1.ActionExecutor();
        this.actionButtonManager = new actions_1.ActionButtonManager(this.actionExecutor);
        this.previewManager = new actions_1.PreviewManager(context);
        this.contextMenuManager = new actions_1.ContextMenuManager(context);
        this.undoManager = new actions_1.UndoManager(context);
        ...
        this.panel = vscode.window.createWebviewPanel('codeAssistChat', 'Code Assistant AI', vscode.ViewColumn.Two, {
            enableScripts: true,
    ...
    setupWebview() {
        ...
        // Send initial state
        this.postMessage({
            type: types_1.WebviewMessageType.INIT,
            payload: {
    ...
    setupMessageHandling() {
        this.panel.webview.onDidReceiveMessage(async (message) => {
            try {
                ...
                this.postMessage({
                    type: types_1.WebviewMessageType.ERROR,
                    payload: {
    ...
    async handleWebviewMessage(message) {
        switch (message.type) {
            case types_1.WebviewMessageType.SEND_MESSAGE:
                await this.handleSendMessage(message.payload);
                break;
            case types_1.WebviewMessageType.ATTACH_FILE:
                await this.handleAttachFile();
                break;
            case types_1.WebviewMessageType.MENTION_FILE:
                await this.handleMentionFile();
                break;
            case types_1.WebviewMessageType.CLEAR_CHAT:
                this.handleClearChat();
                break;
            case types_1.WebviewMessageType.EXPORT_CHAT:
                await this.handleExportChat();
                break;
            case types_1.WebviewMessageType.ACTION_BUTTON_CLICK:
                await this.handleActionButtonClick(message.payload);
                break;
            case types_1.WebviewMessageType.CONTEXT_MENU_ACTION:
                await this.handleContextMenuAction(message.payload);
                break;
            case types_1.WebviewMessageType.UNDO_ACTION:
                await this.handleUndoAction();
                break;
            case types_1.WebviewMessageType.REDO_ACTION:
                await this.handleRedoAction();
                break;
            case types_1.WebviewMessageType.SHOW_PREVIEW:
    ...
    async handleAttachFile() {
        ...
        if (fileUris && fileUris.length > 0) {
            ...
            this.postMessage({
                type: types_1.WebviewMessageType.FILE_ATTACHED,
     ...
     */
    async handleMentionFile() {
        const openEditors = vscode.window.tabGroups.all
            .flatMap(group => group.tabs)
            .filter(tab => tab.input instanceof vscode.TabInputText)
            .map(tab => ({
            label: path.basename(tab.input.uri.fsPath),
            detail: tab.input.uri.fsPath,
            uri: tab.input.uri
        }));
        ...
        if (selected) {
            this.postMessage({
                type: types_1.WebviewMessageType.MENTION_FILE,
                payload: {
     ...
     */
    async handleActionButtonClick(payload) {
        try {
            ...
            this.postMessage({
                type: types_1.WebviewMessageType.ACTION_BUTTON_UPDATE,
     ...
     */
    async handleShowPreview(payload) {
        try {
    ...
    setLoading(isLoading) {
        ...
        this.postMessage({
            type: isLoading ? types_1.WebviewMessageType.LOADING_START : types_1.WebviewMessageType.LOADING_END,
            payload: { isLoading }
        });
    }
    /**
    ...
    updateWebviewState() {
        this.postMessage({
            type: types_1.WebviewMessageType.CONFIG_UPDATE,
     ...
     */
    async getAttachedFilesContent(filePaths) {
        const filesContent = [];
    ...
    updateTheme() {
        this.postMessage({
            type: types_1.WebviewMessageType.THEME_CHANGED,
