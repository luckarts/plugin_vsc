...
class ContextFilter {
     ...
     */
    filterByLanguage(context, languages) {
        try {
            const languageSet = new Set(languages.map(lang => lang.toLowerCase()));
            const filteredContext = {
                ...context,
                openFiles: context.openFiles.filter(file => languageSet.has(file.language.toLowerCase())),
                recentFiles: context.recentFiles.filter(filePath => {
                    const ext = path.extname(filePath).toLowerCase();
                    return this.getLanguageFromExtension(ext).some(lang => languageSet.has(lang.toLowerCase()));
                })
            };
            ...
            throw new types_1.ContextException('filterByLanguage', 'Failed to filter context by language', error);
        }
    }
    /**
     ...
     * @returns Filtered context
     */
    filterByExtension(context, extensions) {
        try {
            const normalizedExtensions = new Set(extensions.map(ext => ext.startsWith('.') ? ext.toLowerCase() : `.${ext.toLowerCase()}`));
            const filteredContext = {
                ...context,
            ...
            throw new types_1.ContextException('filterByExtension', 'Failed to filter context by extension', error);
        }
    }
    /**
     ...
     * @returns Filtered context
     */
    filterByRelevance(context, threshold) {
        try {
            ...
            throw new types_1.ContextException('filterByRelevance', 'Failed to filter context by relevance', error);
        }
    }
    /**
     ...
     * @returns Filtered context
     */
    filterByFileSize(context, maxSize) {
        try {
            const filteredContext = {
                ...context,
                openFiles: context.openFiles.filter(file => this.estimateFileSize(file.filePath) <= maxSize),
                recentFiles: context.recentFiles.filter(filePath => this.estimateFileSize(filePath) <= maxSize)
            };
            ...
            throw new types_1.ContextException('filterByFileSize', 'Failed to filter context by file size', error);
        }
    }
    /**
     ...
     * @returns Filtered context
     */
    applyMultipleFilters(context, filters) {
        try {
            ...
            throw new types_1.ContextException('applyMultipleFilters', 'Failed to apply multiple filters', error);
        }
    }
    /**
    ...
    filterCodeFilesOnly(context) {
        try {
            ...
            throw new types_1.ContextException('filterCodeFilesOnly', 'Failed to filter code files only', error);
        }
    }
    /**
     ...
     */
    filterRecentlyModified(context, maxAge) {
        try {
            const now = Date.now();
            ...
            throw new types_1.ContextException('filterRecentlyModified', 'Failed to filter recently modified files', error);
        }
    }
    /**
    ...
    createSmartFilter(context) {
        try {
            ...
            throw new types_1.ContextException('createSmartFilter', 'Failed to create smart filter', error);
        }
    }
    /**
     ...
     */
    getFilterStats(originalContext, filteredContext) {
